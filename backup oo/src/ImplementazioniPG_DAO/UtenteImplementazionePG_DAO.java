	...
	public Utente utenteLoggato() {
		...
		try {
			...
			ResultSet rs = st.executeQuery(
					"select * from utente where (username = '" + PaginaIniziale.getNomeLogin() + "'AND password = '"
							+ PaginaIniziale.getPasswordLogin() + "') OR (username = '" + AdminLogin.getNomeLoginAdmin()
							+ "' AND password = '" + AdminLogin.getPasswordLoginAdmin() + "') ");
			while (rs.next()) {
